...
function renderAnchor(templates: string[], seed: number, serviceName: string, villeName: string): string {
  ...
  const template = templates[idx] ?? templates[0] ?? '{service} à {ville}'
  return template
...
function selectMoneyPages(
  ...
  if (selected.length < 3) {
    ...
    while (selected.length < 3) {
      ...
      if (!candidate) { pos += stride; continue }
...
export default function MoneyPageBoost({
  ...
  const sectionLabel = SECTION_LABELS[labelSeed % SECTION_LABELS.length] ?? 'Les plus demandés'
