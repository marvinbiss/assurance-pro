...
export default function DevisConfirmation({
  ...
  return (
    <div className={compact ? 'py-4' : 'py-6 md:py-8'}>
      ...
      >
        <p className={`font-semibold text-charcoal-800 text-center ${compact ? 'text-xs mb-2' : 'text-sm mb-3'}`}>
          Recommandez Assurance Pro
        </p>
        <div className="flex gap-2">
          <a
            href={`https://wa.me/?text=${encodeURIComponent(`J'ai trouvé des artisans de confiance sur Assurance Pro : ${typeof window !== 'undefined' ? window.location.href : 'https://assurance-pro.fr'}`)}`}
            target="_blank"
