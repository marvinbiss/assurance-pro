...
/**
 * Form interactif pour générer un modèle d'attestation décennale au format PDF.
 *
 * Génération 100% client-side via @react-pdf/renderer (pas de backend, pas de
 * coût hébergement, pas de RGPD à gérer car aucune donnée n'est envoyée au serveur).
 */
...
// Lazy-load du wrapper PdfDownloadButton (côté navigateur uniquement —
...
const schema = z.object({
  ...
  plafondGarantie: z.string().min(2).max(50),
})
...
const defaultData: FormData = {
  ...
  siret: '',
  adresse: '',
  metiers: 'Plomberie - Chauffage - Sanitaire',
  zoneGeographique: 'France métropolitaine',
  dateDebut: new Date().toISOString().slice(0, 10),
  dateFin: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
  plafondGarantie: 'Selon contrat (à préciser par votre assureur)',
}

export function AttestationDecennaleForm() {
