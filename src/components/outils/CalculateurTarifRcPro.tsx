...
export function CalculateurTarifRcPro() {
  ...
  const resultat = useMemo(
    () => calculerTarif({ secteur, ca, formeJuridique, effectif, antecedents, franchise, plafond, anciennete }),
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="md:col-span-2">
          <label className="block text-sm font-semibold mb-2" htmlFor="calc-secteur">
            Secteur d&apos;activité
          </label>
          <select
            id="calc-secteur"
        ...
        <div>
          <label className="block text-sm font-semibold mb-2" htmlFor="calc-plafond">
        ...
        <div>
          <label className="block text-sm font-semibold mb-2" htmlFor="calc-anciennete">
            Ancienneté d&apos;activité : <span className="text-blue-700">{anciennete} an{anciennete > 1 ? 's' : ''}</span>
          </label>
          <input
            id="calc-anciennete"
            type="range"
      ...
      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-300 rounded-lg p-6 mt-6">
        <p className="text-sm text-gray-600 mb-2">Estimation tarif RC Pro 2026 (annuel HT)</p>
        ...
        <details className="mt-4 text-xs text-gray-600">
          ...
          <ul className="mt-2 space-y-1 pl-4">
            <li>Tarif base secteur : <strong>{Math.round(resultat.detail.base)} €/an</strong></li>
      ...
      <p className="text-xs text-gray-500 italic">
        ...
        garanties optionnelles (RC exploitation, défense recours, protection juridique, cyber).
      </p>
    </div>
  )
}
