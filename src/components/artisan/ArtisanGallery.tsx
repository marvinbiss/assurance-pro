...
export function ArtisanGallery({ artisan }: ArtisanGalleryProps) {
  ...
  // Safe: we checked photos.length > 0 above
  const firstPhoto = photos[0] as PortfolioItem
  ...
  const currentPhoto = photos[currentIndex] as PortfolioItem | undefined
        ...
        <div className="grid grid-cols-4 grid-rows-2 gap-2 h-60 md:h-80 rounded-xl overflow-hidden">
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
              {firstPhoto.mediaType === 'video' ? (
                <div className="w-16 h-16 rounded-full bg-black/60 flex items-center justify-center">
                  <Play className="w-8 h-8 text-white ml-1" fill="currentColor" />
                </div>
              ) : firstPhoto.mediaType === 'before_after' ? (
