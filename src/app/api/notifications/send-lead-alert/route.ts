...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.assurance-pro.fr'
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const provider of providers) {
      ...
      // Send SMS
      if (provider.phone) {
        const smsMessage = `Nouvelle demande ${service || 'devis'} à ${city || ''}. Répondez vite sur Assurance Pro.fr`
