...
import { decennaleStaticSlugs, rcProStaticSlugs } from '@/lib/seo/garantie-slug-dispatcher'
import { getEligibleSlugsForTemplate } from '@/lib/programmatic/page-enrichment'
...
const STATIC_PAGES: Array<{ path: string; priority: number; changeFrequency: ChangeFrequency }> = [
  ...
  {
    path: 'guides/quelle-assurance-professionnelle-choisir',
    priority: 0.9,
    changeFrequency: 'monthly',
  }, // 700 vol KD 7 (guide hub)
  ...
  {
    path: 'guides/resiliation-assurance-professionnelle',
    priority: 0.8,
    changeFrequency: 'monthly',
  }, // Loi Hamon
  ...
  { path: 'guides/assurance-decennale-immediate', priority: 0.85, changeFrequency: 'monthly' }, // 150 vol KD 0 CPC 700€ — intent urgence chantier
  {
    ...
    priority: 0.85,
    changeFrequency: 'monthly',
  }, // 100 vol KD 0 CPC 1 300€ ⭐ CPC #1 marché entier
...
// ────────────────────────────────────────────────────────────────────────────
// Programmatic C+D routes (migration 015 — v_page_enrichment_full)
// ────────────────────────────────────────────────────────────────────────────
// Lit page_enrichment_cache. Si cache vide → retourne [] (pas d'erreur).
// Cache populé par scripts d'ingest (à venir).
// ────────────────────────────────────────────────────────────────────────────
...
async function getProgrammaticEntries(now: Date): Promise<MetadataRoute.Sitemap> {
  try {
    ...
    // Defensive : si DB indisponible ou cache vide, on ne casse pas le sitemap.
    console.error('[sitemap] programmatic entries error', err)
    return []
  }
}
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const verticales: Array<
    'multirisque-pro' | 'mutuelle-pro' | 'assurance-vtc' | 'cyber-assurance'
  > = ['multirisque-pro', 'mutuelle-pro', 'assurance-vtc', 'cyber-assurance']
