...
export function generateMetadata({ params }: { params: Params }): Metadata {
  ...
  if (r.kind === 'profession') {
    ...
    return {
      title: `RC Pro ${p.name} 2026 — Tarifs + Comparatif | Assurance Pro`,
  ...
  const v = r.ville
  return {
    title: `RC Pro ${v.nom} (${v.departementCode}) — Freelances, consultants, agences | Assurance Pro`,
...
export default function RcProSlugPage({ params }: { params: Params }) {
  const r = resolveRcProSlug(params.slug)
  if (!r) notFound()
  ...
  const tarifAutoEnt = p.tarifs.auto_entrepreneur.max > 0
    ? p.tarifs.auto_entrepreneur
    : p.tarifs.sarl_sas
  const tarifLabel = p.tarifs.auto_entrepreneur.max > 0 ? 'Tarif auto-entrepreneur' : 'Tarif SARL/SAS'
  ...
  return (
    <PilierLayout
      ...
      intro={p.intro}
      benefits={[
        { icon: p.icon, title: p.name, desc: `~${p.nbProsFrance.toLocaleString('fr-FR')} en France` },
        { icon: '🛡️', title: `Plafond ${p.plafondRecommande}M€`, desc: 'Recommandé pour ce métier' },
      ...
      sections={[
        {
          ...
          body: (
            <>
              ...
              <p>Sans RC Pro, vous engagez votre patrimoine personnel (auto-entrepreneur, EI) ou celui de votre société en cas de mise en cause par un client mécontent. Un litige peut représenter plusieurs dizaines voire centaines de milliers d'euros (frais d'avocats, dommages-intérêts, perte d'exploitation).</p>
            </>
          ),
        },
        {
              ...
              <table className="w-full text-sm border-collapse my-4">
                ...
                <tbody>
                  {p.tarifs.auto_entrepreneur.max > 0 ? (
                    <tr>
                      <td className="border border-gray-300 p-2"><strong>Auto-entrepreneur</strong></td>
                      <td className="border border-gray-300 p-2 text-right">{p.tarifs.auto_entrepreneur.min}€ - {p.tarifs.auto_entrepreneur.max}€</td>
                    </tr>
                  ) : null}
                  {p.tarifs.sarl_sas.max > 0 ? (
                    <tr>
                      <td className="border border-gray-300 p-2"><strong>SARL / SAS / SASU</strong></td>
                      <td className="border border-gray-300 p-2 text-right">{p.tarifs.sarl_sas.min}€ - {p.tarifs.sarl_sas.max}€</td>
                    </tr>
                  ) : null}
                  {p.tarifs.grand_compte.max > 0 ? (
                    <tr>
                      <td className="border border-gray-300 p-2"><strong>Grand compte</strong> (CA &gt; 500k€)</td>
                      <td className="border border-gray-300 p-2 text-right">{p.tarifs.grand_compte.min}€ - {p.tarifs.grand_compte.max}€</td>
                    </tr>
                  ) : null}
                </tbody>
              </table>
              <p className="text-xs italic text-gray-600">
                Tarifs indicatifs basés sur les barèmes 2026 de nos 10 assureurs partenaires. Le tarif réel dépend
                de votre profil (CA, ancienneté, garanties souhaitées) et fait l\'objet d\'un devis personnalisé.
              </p>
            </>
          ),
        },
