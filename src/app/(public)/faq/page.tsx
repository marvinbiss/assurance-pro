...
export const metadata: Metadata = {
  ...
  alternates: { canonical: `${process.env.NEXT_PUBLIC_SITE_URL ?? 'https://assurance-pro.fr'}/faq` },
}
    ...
    <main className="min-h-screen bg-white py-12">
      <div className="container mx-auto px-4 max-w-4xl">
        <header className="mb-10 text-center">
          <h1 className="text-3xl md:text-4xl font-bold mb-3">FAQ — Assurance Pro</h1>
          <p className="text-gray-600 text-lg">
            Réponses claires aux questions les plus fréquentes sur l\'assurance professionnelle.
          </p>
        </header>

        <nav className="mb-10 flex flex-wrap gap-2 border-y py-4 justify-center">
          {CATEGORIES.map((cat) => (
            <a
              key={cat}
              href={`#cat-${cat.replace(/[^a-z0-9]/gi, '-').toLowerCase()}`}
              className="px-3 py-1 bg-blue-50 text-blue-700 rounded font-semibold hover:bg-blue-100 text-sm"
            >
              {cat}
            </a>
          ))}
        ...
        <div className="space-y-12">
          {CATEGORIES.map((cat) => (
            <section key={cat} id={`cat-${cat.replace(/[^a-z0-9]/gi, '-').toLowerCase()}`}>
              <h2 className="text-2xl font-bold mb-4 pb-2 border-b">{cat}</h2>
              <div className="space-y-3">
                {FAQ.filter((f) => f.category === cat).map((f) => (
                  <details key={f.q} className="bg-gray-50 rounded-lg p-5 cursor-pointer group">
                    <summary className="font-semibold text-gray-900 list-none flex justify-between items-start gap-4">
                      <span>{f.q}</span>
                      <span className="text-blue-600 group-open:rotate-180 transition flex-shrink-0" aria-hidden="true">▼</span>
                    </summary>
        ...
        <div className="mt-16 text-center bg-blue-50 p-8 rounded-lg">
          <h2 className="text-xl font-bold mb-3">Votre question n\'est pas listée&nbsp;?</h2>
