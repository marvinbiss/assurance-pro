...
function parseContentBlocks(content: string[]): ParsedBlock[] {
  ...
  while (i < allLines.length) {
    ...
    if (line.startsWith(':::') && !line.startsWith(':::end') && line !== ':::') {
      ...
      if (calloutMatch) {
        const calloutType = (calloutMatch[1] ?? 'info') as CalloutBlock['calloutType']
        const title = (calloutMatch[2] ?? '').trim()
        const calloutContent: string[] = []
        i++
        while (i < allLines.length && allLines[i] !== ':::') {
          const cl = allLines[i]
          if (cl) calloutContent.push(cl)
          i++
        }
        // Skip the closing :::
...
function getAuthorGradient(name: string): string {
  ...
  return gradients[Math.abs(hash) % gradients.length] ?? 'from-blue-500 to-blue-600'
}
...
export default async function BlogArticlePage({ params }: PageProps) {
  ...
  const primaryServiceSlug = firstServiceLink
    ? firstServiceLink.href.split('/services/')[1]?.split('/')[0] ?? null
    : null
                ...
                <div className="flex flex-col sm:flex-row gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-600 text-sm leading-relaxed mb-3">
                      {authorProfile
                        ? authorProfile.bio
                        : (article.authorBio || "Expert en artisanat et batiment chez Assurance Pro. Nos contenus sont rediges en collaboration avec des professionnels du secteur et verifies pour leur exactitude technique.")}
                    </p>
                    {authorProfile && (
                      <>
                        {/* Expertise tags */}
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                          ...
                          >
                            Voir tous les articles de {article.author.split(' ')[0] ?? article.author}
                          </Link>
                        </div>
                      </>
                    )}
