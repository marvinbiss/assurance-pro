...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${tagInfo.label} — Articles & Guides | Assurance Pro`
  const description = `Tous les articles sur ${tagInfo.label.toLowerCase()} : conseils, prix, réglementation et guides pratiques par les experts Assurance Pro.`
...
export default async function BlogTagPage({ params }: PageProps) {
  ...
  const collectionSchema = {
    ...
    hasPart: articles.slice(0, 10).map(a => ({
      ...
      author: (() => {
        const authorName = allArticles[a.slug]?.author || 'Assurance Pro'
        return authorName === 'Assurance Pro'
          ? { '@type': 'Organization', name: 'Équipe éditoriale Assurance Pro', url: `${SITE_URL}/a-propos`, '@id': `${SITE_URL}#organization` }
