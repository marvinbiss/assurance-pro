...
function villeToPartialCommuneData(ville: Ville): CommuneData {
  ...
  return {
    ...
    revenu_median: REGIONAL_REVENU_MEDIAN[ville.region] != null
    ...
    prix_m2_moyen: getVilleRealPrix(ville.slug)
      ?? (REGIONAL_PRIX_M2[ville.region] != null
...
function generateIntroText(
  ...
  const templates = [
    ...
    `${ville.name} compte ${count} ${svcLower}s référencés sur Assurance Pro. Située dans le département ${dep} en région ${region}, cette commune de ${pop} habitants bénéficie d'un réseau dense de professionnels du bâtiment, tous identifiés par leur numéro SIREN via l'API officielle du gouvernement.`,

    // 2 — Annuaire et code postal
    `Assurance Pro recense ${count} ${svcLower}s à ${ville.name} (${ville.codePostal}). Cette commune de ${pop} habitants du département ${dep}, en région ${region}, dispose de professionnels référencés dans votre secteur géographique. Chaque ${svcLower} listé est identifié par son numéro SIREN, garantissant une activité déclarée et vérifiable.`,

    // 3 — Exercent dans le département
    `${countSpace}${svcLower}s exercent à ${ville.name}, dans le département ${dep}. Cette ville de ${pop} habitants en ${region} dispose de nombreux artisans qualifiés. Assurance Pro vous permet de comparer les ${svcLower}s référencés à ${ville.name} (${ville.codePostal}), de consulter leurs coordonnées et de les contacter directement pour un devis gratuit.`,
    ...
    `À ${ville.name}, ${count} ${svcLower}s sont référencés par numéro SIREN. Les habitants de cette commune (${pop} habitants, ${dep}, ${region}) peuvent compter sur Assurance Pro pour comparer les professionnels exerçant à ${ville.name} et dans les communes voisines du ${ville.departementCode}. Chaque profil est vérifié via le registre SIREN, vous assurant de contacter des artisans en règle.`,

    // 6 — Intervention et code postal
    `${count} ${svcLower}s interviennent à ${ville.name} (${ville.codePostal}), département ${dep}. Cette commune en ${region} rassemble ${pop} habitants qui peuvent avoir besoin d'un professionnel pour leurs projets de construction, rénovation ou dépannage. Assurance Pro met à votre disposition un annuaire de professionnels vérifiés par SIREN intervenant à ${ville.name} et ses alentours.`,

    // 7 — Identification SIREN par département
    `${ville.name} (${dep}) dispose de ${count} ${svcLower}s identifiés par SIREN. Le métier de ${svcLower} requiert un savoir-faire spécifique, et dans cette commune de ${pop} habitants, les professionnels du secteur ne manquent pas. Assurance Pro vous aide à les identifier en ${region}, chacun vérifié auprès des données officielles de l'État.`,

    // 8 — Devis gratuit
    `Assurance Pro liste ${count} ${svcLower}s à ${ville.name} pour un devis gratuit. Notre annuaire recense les artisans identifiés par SIREN dans le ${dep} (${ville.departementCode}), en région ${region}. Avec ${pop} habitants, ${ville.name} dispose d'un tissu artisanal dynamique pour répondre à toutes vos demandes.`,

    // 9 — Actifs et population
    `${count} ${svcLower}s sont actifs à ${ville.name}, commune de ${pop} habitants. Dans le ${dep}, la demande en services de ${svcLower} reste constante tout au long de l'année. En ${region}, Assurance Pro référence les professionnels intervenant à ${ville.name} (${ville.codePostal}) et dans les communes environnantes, tous identifiés par leur numéro SIREN.`,

    // 10 — Trouvez un professionnel
    `Trouvez un ${svcLower} vérifié à ${ville.name} parmi ${count} professionnels référencés. Que ce soit pour un dépannage urgent ou un projet planifié, Assurance Pro répertorie les artisans du ${dep} (${ville.codePostal}) en ${region}, vérifiés via l'API Sirene du gouvernement, pour vous permettre de comparer et de choisir en toute confiance.`,
    ...
    `Comparez les ${count} ${svcLower}s référencés à ${ville.name} (${ville.codePostal}). Assurance Pro liste les professionnels vérifiés dans le ${dep} (${region}), pour cette commune de ${pop} habitants. Consultez les coordonnées de chaque artisan identifié par SIREN et demandez plusieurs devis gratuits pour votre projet.`,

    // 13 — Disponibilité locale
    `${countSpace}${svcLower}s qualifiés sont disponibles à ${ville.name}, ${dep}. La qualité des travaux dépend avant tout du choix du professionnel. Dans cette commune de ${pop} habitants en ${region}, Assurance Pro vous propose un annuaire d'artisans référencés et vérifiés par leur numéro SIREN, pour sélectionner le bon prestataire en toute sérénité.`,

    // 14 — Proximité géographique
    `${count} ${svcLower}s proches de ${ville.name} (${ville.codePostal}) sont sur Assurance Pro. La proximité est essentielle pour bénéficier d'une intervention rapide et de tarifs compétitifs. Notre annuaire couvre tout le département ${dep} en ${region} et référence les professionnels prêts à intervenir dans votre commune de ${pop} habitants et ses environs.`,

    // 15 — Vérifiés et comparables
    `${ville.name} (${ville.codePostal}) : ${count} ${svcLower}s vérifiés et comparables. Dans le ${dep} (${region}), chaque artisan référencé sur Assurance Pro est identifié par son numéro SIREN via les données officielles du gouvernement, un critère de fiabilité essentiel pour les ${pop} habitants de cette commune.`,
  ]
...
function generateLocalTips(
  ...
  if (trade && trade.tips.length > 0) {
    ...
    const tip1 = trade.tips[idx1]
    if (tip1) tips.push(tip1)

    if (trade.tips.length > 2) {
      const idx2 = (hash + 3) % trade.tips.length
      if (idx2 !== idx1) {
        const tip2 = trade.tips[idx2]
        if (tip2) tips.push(tip2)
      }
    }
  }
...
function generateConclusion(
  ...
  const templates = [
    ...
    `Ne perdez plus de temps à chercher un ${svcLower} fiable à ${ville.name}. Consultez les ${countSpace}profils référencés sur Assurance Pro, comparez les coordonnées et contactez directement le professionnel de votre choix dans le ${ville.departement} (${ville.departementCode}).${urgencyLine}${certLine}`,

    // 2 — Simplicité de la démarche
    `Assurance Pro simplifie votre recherche de ${svcLower} à ${ville.name} (${ville.codePostal}). Parcourez notre annuaire de ${countSpace}professionnels référencés en ${ville.region}, consultez leurs informations et demandez vos devis gratuitement.${urgencyLine}${certLine}`,
    ...
    `Faites le bon choix pour vos travaux à ${ville.name} (${ville.codePostal}). Les ${countSpace}${svcLower}s référencés sur Assurance Pro dans le ${ville.departement} sont tous identifiés par SIREN, un gage de sérieux et de transparence pour les habitants de ${ville.name}.${urgencyLine}${certLine}`,

    // 5 — Recommandation d'action
    `N'attendez plus pour lancer votre projet : consultez dès maintenant les ${countSpace}${svcLower}s disponibles à ${ville.name} et dans le ${ville.departementCode}. Assurance Pro vous donne accès aux coordonnées de professionnels vérifiés en ${ville.region}, pour des devis rapides et sans engagement.${urgencyLine}${certLine}`,

    // 6 — Résumé des avantages
    `Annuaire vérifié, données SIREN officielles, accès gratuit : Assurance Pro réunit toutes les conditions pour vous aider à trouver un ${svcLower} de confiance à ${ville.name} (${ville.codePostal}). Comparez les ${countSpace}professionnels du ${ville.departement} et contactez celui qui correspond à votre besoin.${urgencyLine}${certLine}`,

    // 7 — Proximité et réactivité
    `À ${ville.name}, un ${svcLower} qualifié n'est qu'à quelques clics. Parcourez les ${countSpace}profils référencés dans le ${ville.departement} (${ville.region}) sur Assurance Pro et trouvez le professionnel idéal pour votre projet, qu'il s'agisse d'un dépannage ou de travaux planifiés.${urgencyLine}${certLine}`,
    ...
    // 9 — Question finale
    `Prêt à trouver votre ${svcLower} à ${ville.name} ? Assurance Pro met à votre disposition ${countSpace}professionnels référencés dans le ${ville.departement} (${ville.departementCode}), en ${ville.region}. Comparez les profils, vérifiez les qualifications et lancez votre projet en toute sérénité.${urgencyLine}${certLine}`,

    // 10 — Mise en avant locale
    `Les ${countSpace}${svcLower}s de ${ville.name} référencés sur Assurance Pro sont prêts à intervenir dans votre commune et dans tout le ${ville.departement}. Profitez d'un annuaire fiable, basé sur les données SIREN du gouvernement, pour choisir le bon professionnel en ${ville.region}.${urgencyLine}${certLine}`,
  ]
...
const SVC_LOCATION_FAQ_POOL: { q: (p: SvcLocFaqParams) => string; a: (p: SvcLocFaqParams) => string }[] = [
  ...
  },
  {
    ...
    a: (p) => `Sur Assurance Pro, les ${p.svc}s référencés à ${p.name} sont vérifiés via leur numéro SIREN. Comparez les profils, vérifiez les qualifications et demandez jusqu'à 3 devis gratuits.`,
  },
  {
  ...
  },
  {
    ...
    a: (p) => `Les ${p.svc}s référencés sur Assurance Pro couvrent l'ensemble des quartiers de ${p.name} et ses environs dans le ${p.dept} (${p.deptCode}). Consultez la section quartiers sur cette page.`,
  },
  {
  ...
  },
  {
    ...
    a: (p) => `Oui, tous les ${p.svc}s listés sur Assurance Pro à ${p.name} sont identifiés par leur numéro SIREN via les données officielles du gouvernement. C'est un gage de transparence et de fiabilité.`,
  },
]
...
function generateServiceClimateTip(svc: string, cityName: string, climate: ClimateZone, seed: number): string {
  ...
  const tipFn = tips[seed % tips.length]
  return tipFn ? tipFn(svc, cityName) : ''
}
...
function getQuartierProfile(ville: Ville, quartierName: string): QuartierProfile {
  ...
  if (ville.departementCode === '75') {
    ...
    const arrNum = arrMatch?.[1] ? parseInt(arrMatch[1], 10) : 0
...
function generateQuartierDataDrivenContent(
  ...
  if (gel != null || precip != null) {
    ...
    if (impactFns) {
      ...
      if (impactFn) climatParts.push(impactFn(quartierName))
    }
...
export function generateQuartierContent(ville: Ville, quartierName: string, serviceSlug?: string): QuartierContent {
  ...
  const ctxFn = ctxTemplates?.[Math.abs(hashCode(`ctx-${ville.slug}-${quartierName}${seedSuffix}`)) % ctxTemplates.length]
  const batimentContext = ctxFn ? ctxFn(quartierName, ville.name) : ''
  ...
  const tipFn = tipTemplates?.[Math.abs(hashCode(`tips-${ville.slug}-${quartierName}${seedSuffix}`)) % tipTemplates.length]
  const conseils = tipFn ? tipFn(quartierName, ville.name) : ''
...
const DEPT_FAQ_POOL: { q: (name: string, code: string) => string; a: (name: string, code: string, region: string, pop: string, climate: string, issues: string[]) => string }[] = [
  ...
  { q: () => `Les devis sont-ils gratuits et sans engagement ?`, a: (name) => `Oui, les devis via Assurance Pro pour le ${name} sont gratuits et sans engagement. Comparez jusqu'à 3 propositions de professionnels qualifiés.` },
...
function getDepartementProfile(dept: import('@/lib/data/france').Departement): DepartementProfile {
  ...
  while (selectedIssues.length < 5 && selectedIssues.length < allIssues.length) {
    ...
    if (!used.has(idx) && issue) { used.add(idx); selectedIssues.push(issue) }
...
export function generateDepartementContent(dept: import('@/lib/data/france').Departement): DepartementContent {
  ...
  const deptTipFn = tipTemplates?.[Math.abs(hashCode(`tips-dept-${dept.slug}`)) % tipTemplates.length]
  const conseilsDepartement = deptTipFn ? deptTipFn(dept.name, dept.code) : ''
...
const REGION_FAQ_POOL: { q: (name: string) => string; a: (name: string, deptCount: number, cityCount: number, climate: string, facts: string[]) => string }[] = [
  ...
  { q: (name) => `Les devis sont-ils gratuits en ${name} ?`, a: (name) => `Oui, tous les devis via Assurance Pro pour ${name} sont 100% gratuits et sans engagement. Jusqu'à 3 propositions de professionnels qualifiés.` },
...
function getRegionProfile(region: import('@/lib/data/france').Region): RegionProfile {
  ...
  const mixedFacts: string[] = [...facts.slice(0, 3), climateIssues[seed % climateIssues.length] ?? '', climateIssues[(seed + 1) % climateIssues.length] ?? '']
...
export function generateRegionContent(region: import('@/lib/data/france').Region, cityCountOverride?: number): RegionContent {
  ...
  const ctxRegionFn = ctxTemplates?.[Math.abs(hashCode(`ctx-region-${region.slug}`)) % ctxTemplates.length]
  const contexteRegional = ctxRegionFn ? ctxRegionFn(region.name, deptCount, cityCount) : ''
  ...
  const regionTipFn = tipTemplates?.[Math.abs(hashCode(`tips-region-${region.slug}`)) % tipTemplates.length]
  const conseilsRegion = regionTipFn ? regionTipFn(region.name) : ''
...
function getVilleProfile(ville: import('@/lib/data/france').Ville): VilleProfile {
  ...
  while (selectedIssues.length < 5 && selectedIssues.length < allIssues.length) {
    ...
    const issue = allIssues[idx]
    if (!usedIdx.has(idx) && issue) { usedIdx.add(idx); selectedIssues.push(issue) }
...
const VILLE_FAQ_POOL: { q: (name: string) => string; a: (p: VilleFaqParams) => string }[] = [
  {
    ...
    a: (p) => `Sur Assurance Pro, tous les artisans référencés à ${p.name} disposent d'un numéro SIREN vérifié. Sélectionnez le corps de métier souhaité, consultez les profils et demandez jusqu'à 3 devis gratuits pour comparer les offres.`,
  },
  {
  ...
  },
  {
    ...
    a: (p) => `Les artisans référencés sur Assurance Pro sont répertoriés à partir des données SIREN officielles de l'INSEE. Chaque professionnel listé à ${p.name} dispose d'un numéro SIREN enregistré et vérifiable.`,
  },
  {
...
export function generateVilleContent(ville: import('@/lib/data/france').Ville): VilleContent {
  ...
  const villeCtxFn = ctxTemplates[Math.abs(hashCode(`ctx-ville-${ville.slug}`)) % ctxTemplates.length]!
  const contexteUrbain = villeCtxFn({ name: ville.name, pop: ville.population, qc: quartierCount })
  ...
  const villeTipFn = tipTemplates?.[Math.abs(hashCode(`tips-ville-${ville.slug}`)) % tipTemplates.length]
  const conseilsVille = villeTipFn ? villeTipFn(ville.name) : ''
