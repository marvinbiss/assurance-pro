...
export function generateDataDrivenContent(
  ...
  const introTemplates = commune.gentile
    ? [
        `Les ${commune.gentile} à la recherche d'un ${svc} peuvent compter sur Assurance Pro.`,
        `Habitants de ${commune.name}, trouvez votre ${svc} sur Assurance Pro, l'annuaire des artisans vérifiés.`,
        `${commune.gentile}, besoin d'un ${svc} ? Notre plateforme référence les professionnels qualifiés de votre commune.`,
      ]
    : [
        `Vous cherchez un ${svc} à ${commune.name} ? Assurance Pro vous accompagne.`,
        ...
        `${commune.name} : comparez les ${svc}s référencés sur Assurance Pro et contactez-les directement.`,
      ]
  introParts.push(introTemplates[seed % introTemplates.length] ?? introTemplates[0] ?? '')
  ...
  if (trade) {
    if (trade.emergencyInfo) {
      ...
      introParts.push(
        `Les ${svc}s de notre annuaire interviennent à ${commune.name} pour tous vos projets de ${trade.commonTasks.length > 0 ? (trade.commonTasks[0]?.split(' : ')[0]?.toLowerCase() ?? 'travaux') : 'travaux'} et bien plus.`
      )
    }
  }
  ...
  if (commune.revenu_median || commune.nb_logements || commune.part_maisons_pct !== null) {
    ...
    if (commune.revenu_median) {
      ...
      parts.push(revenuTemplates[seed % revenuTemplates.length] ?? revenuTemplates[0] ?? '')
    ...
    if (commune.nb_logements) {
      ...
      parts.push(logTemplates[(seed + 1) % logTemplates.length] ?? logTemplates[0] ?? '')
    }
  ...
  if (commune.prix_m2_moyen || commune.prix_m2_maison || commune.prix_m2_appartement) {
    ...
    if (commune.prix_m2_moyen) {
      ...
      parts.push(immoTemplates[seed % immoTemplates.length] ?? immoTemplates[0] ?? '')
    }
    ...
    if (commune.nb_transactions_annuelles) {
      ...
      parts.push(transTemplates[(seed + 2) % transTemplates.length] ?? transTemplates[0] ?? '')
    }
  ...
  if (commune.nb_entreprises_artisanales || commune.nb_artisans_btp || commune.nb_artisans_rge) {
    ...
    if (commune.nb_entreprises_artisanales) {
      ...
      parts.push(artTemplates[seed % artTemplates.length] ?? artTemplates[0] ?? '')
      ...
      if (commune.nb_entreprises_artisanales > 0 && pop > 0) {
        ...
        parts.push(ratioTemplates[(seed + 1) % ratioTemplates.length] ?? ratioTemplates[0] ?? '')
      }
    }
  ...
  if (commune.pct_passoires_dpe !== null && commune.pct_passoires_dpe !== undefined) {
    ...
    parts.push(dpeTemplates[seed % dpeTemplates.length] ?? dpeTemplates[0] ?? '')
  ...
  if (commune.jours_gel_annuels !== null || commune.precipitation_annuelle !== null ||
    ...
    if (commune.climat_zone) {
      ...
      parts.push(climTemplates[seed % climTemplates.length] ?? climTemplates[0] ?? '')
    }
    ...
    if (commune.temperature_moyenne_hiver !== null && commune.temperature_moyenne_ete !== null) {
      ...
      parts.push(tempTemplates[(seed + 1) % tempTemplates.length] ?? tempTemplates[0] ?? '')
    }
    ...
    if (commune.jours_gel_annuels !== null) {
      ...
      parts.push(gelTemplates[(seed + 2) % gelTemplates.length] ?? gelTemplates[0] ?? '')
    ...
    if (commune.precipitation_annuelle !== null) {
      ...
      parts.push(precipTemplates[seed % precipTemplates.length] ?? precipTemplates[0] ?? '')
  ...
  if (interiorTrades.includes(serviceSlug)) {
    ...
    demandeLocaleParts.push(
      `À ${commune.name}, en ${commune.region_name || 'France'}, les besoins en ${svc} sont liés à l'entretien courant du patrimoine bâti et aux projets d'amélioration de l'habitat. Les artisans référencés sur Assurance Pro interviennent dans un périmètre adapté à la demande locale.`
    )
  }
  ...
  if (trade) {
    ...
    let costAnswer = costOpenTemplates[seed % costOpenTemplates.length] ?? costOpenTemplates[0] ?? ''
  ...
  if (commune.nb_entreprises_artisanales || commune.nb_artisans_btp) {
    ...
    if (providerCount > 0) {
      answer += ` Assurance Pro référence ${providerCount} ${svc}${providerCount > 1 ? 's' : ''} vérifiés dans la commune.`
    }
    faqItems.push({
  ...
  const verifTemplates = [
    ...
    `Sur Assurance Pro, les ${svc}s de ${commune.name} (${commune.departement_code}) font l'objet d'une vérification SIREN systématique. Numéro d'immatriculation, statut d'activité et code APE sont contrôlés pour garantir la fiabilité de l'annuaire.`,
  ]
  faqItems.push({
    question: `Les ${svc}s à ${commune.name} sont-ils vérifiés ?`,
    answer: verifTemplates[(seed + 1) % verifTemplates.length] ?? verifTemplates[0] ?? '',
  })
