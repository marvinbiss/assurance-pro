/**
 * Analytics & Conversion Tracking - Assurance Pro
...
export function analyzeFunnel(stepCounts: number[]): FunnelAnalysis[] {
  ...
  return steps.map((step, index) => ({
    ...
    dropOffRate:
      index === 0
        ? 0
        : (stepCounts[index - 1] ?? 0) > 0
        ? Math.round(
            (((stepCounts[index - 1] ?? 0) - (stepCounts[index] ?? 0)) / (stepCounts[index - 1] ?? 1)) * 100
          )
        : 0,
  }))
}
...
export function getVariant(experimentId: string, variants: string[]): string {
  ...
  let variant: string | null = localStorage.getItem(storageKey)
