...
function computeCodePostal(
  ...
  if (slug === 'lyon') {
    ...
    return lyonCodes[quartierIndex % lyonCodes.length] ?? '69001'
  }
...
function generateDescription(
  ...
  const template = DESC_TEMPLATES[templateIdx]

  return template ? template(quartierName, villeName, eraAdj, typeAdj) : `${quartierName} est un quartier de ${villeName}.`
}
...
function generateAtout(
  ...
  if (epoque === 'post-2000' && (typeQuartier === 'résidentiel' || typeQuartier === 'mixte')) {
    return ATOUT_RECENT_BBC[h % ATOUT_RECENT_BBC.length] ?? ATOUT_RECENT_BBC[0] ?? ''
  }
  if (typeQuartier === 'populaire' && transport.includes('metro')) {
    return ATOUT_METRO[h % ATOUT_METRO.length] ?? ATOUT_METRO[0] ?? ''
  }
