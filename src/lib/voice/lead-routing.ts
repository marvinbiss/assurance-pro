/**
 * Voice Lead Routing — Assurance Pro
 *
...
export async function createVoiceLead(
  ...
  const { data: lead, error: leadError } = await admin
    ...
    .insert({
      ...
      client_email: (qualData.email as string) ?? `voice+${voiceCall.id.slice(0, 8)}@assurance-pro.fr`,
  ...
  if (artisan.phone) {
    ...
    const smsMessage =
      `Nouveau lead Assurance Pro !\n` +
      `${serviceName}${location}\n` +
      `Client : ${callerName}\n` +
      `Connectez-vous pour répondre : assurance-pro.fr/espace-artisan/leads`
